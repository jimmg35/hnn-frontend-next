...
const MapPopup = ({
  ...
  const transformStyle = (function () {
    ...
    const x = mapRef.current.offsetLeft + transform.x - width / 2
    const y = mapRef.current.offsetTop + transform.y - height - 15
  ...
  const subscribeWatchPosition = () => {
    if (cancelAnimationFrame.current) return
      ...
      (
        open
          ? <div ref={elemRef} style={{
            transform: transformStyle, position: 'absolute',
            left: '0px', top: '0px', zIndex: 50
          }}>
